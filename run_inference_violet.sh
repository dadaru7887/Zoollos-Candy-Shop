...
EVAL_DIR="$HOME/eval_single_violet"
if [ -d "$EVAL_DIR" ]; then
  rm -rf "$EVAL_DIR"
fi
...
cd ~/il_ws/src/lerobot && lerobot-record \
  ...
  --display_data=true \
  --dataset.repo_id=${HF_USER}/eval_single_violet \
  --dataset.root="$HOME/eval_single_violet" \
  --dataset.single_task="Pick up Single Violet" \
  --policy.path="$HOME/il_ws/src/lerobot/outputs/train/single_violet_policy/checkpoints/last/pretrained_model" \
